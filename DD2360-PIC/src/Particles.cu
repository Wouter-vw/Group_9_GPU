...
void unflattenArray(FPpart* array1D, FPpart*** array3D, int nx, int ny, int nz) {
...
__global__ void mover_PC_kernel(
    ...
    FPpart d_Lx, FPpart d_Ly, FPpart d_Lz, FPpart dt_sub_cycling, FPpart dto2, FPpart qomdt2,
    bool d_PERIODICX, bool d_PERIODICY, bool d_PERIODICZ, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop, int d_NiterMover) 
  {
    ...
    if (i < d_nop) {
      ...
      for (int inner = 0; inner < d_NiterMover; inner++) {
          ...
          // Check indixing
          xi[0] = d_x[i] - d_XN_flat[(ix - 1) * (d_nyn * d_nzn) + (iy) * d_nzn + (iz)];
...
int mover_PC_GPU(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param) {
    ...
    cudaMemcpy(d_XN_flat, XN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(d_YN_flat, YN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(d_ZN_flat, ZN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Ex_flat, Ex_flat, nxn * nyn * nzn * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Ey_flat, Ey_flat, nxn * nyn * nzn * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Ez_flat, Ez_flat, nxn * nyn * nzn * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Bxn_flat, Bxn_flat, nxn * nyn * nzn * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Byn_flat, Byn_flat, nxn * nyn * nzn * sizeof(FPfield), cudaMemcpyHostToDevice);
    cudaMemcpy(d_Bzn_flat, Bzn_flat, nxn * nyn * nzn * sizeof(FPfield), cudaMemcpyHostToDevice);
    ...
    // Loop over all sub cycles. Wait untill finished for a number of points before you go to the next
    for (int i_sub=0; i_sub < part->n_sub_cycles; i_sub++) {
      mover_PC_kernel<<<blocksPerGrid, threadsPerBlock>>>(
          d_x, d_y, d_z,
          d_u, d_v, d_w,
          d_XN_flat, d_YN_flat, d_ZN_flat,
          d_Ex_flat, d_Ey_flat, d_Ez_flat,
          d_Bxn_flat, d_Byn_flat, d_Bzn_flat,
          ...
          nxn, nyn, nzn, nop, part->NiterMover
      );
...
__device__  void call_weight(int ix, int iy, int iz, FPpart x, FPpart y, FPpart z, FPpart u, FPpart v, FPpart w, FPpart q, 
    FPpart* d_XN_flat, FPpart* d_YN_flat, FPpart* d_ZN_flat, FPpart d_invVOL, 
    ...
    for (int ii = 0; ii < 2; ii++) {
        for (int jj = 0; jj < 2; jj++) {
            for (int kk = 0; kk < 2; kk++) {
                weight[ii][jj][kk] = q * xi[ii] * eta[jj] * zeta[kk] * d_invVOL * d_invVOL;
            }
        }
    }
...
__global__ void calculate_weight(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_q,
    FPpart* d_XN_flat, FPpart* d_YN_flat, FPpart* d_ZN_flat, 
    FPpart* d_weight,
...
__global__ void interpP2G_kernel_rhon(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    ...
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
    ...
    if (i < d_nop) {
        ...
        for (int ii = 0; ii < 2; ii++) {
            for (int jj = 0; jj < 2; jj++) {
                for (int kk = 0; kk < 2; kk++) {
                    int index = (ix - ii) * (d_nyn * d_nzn) + (iy - jj) * d_nzn + (iz - kk);
                    int ind = 8 * i + 4 * ii + 2 * jj + kk;
...
__global__ void interpP2G_kernel_Jx(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_u, 
    FPpart* d_Jx_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_Jy(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_v,
    FPpart* d_Jy_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_Jz(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_w,
    FPpart* d_Jz_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_pxx(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_u,
    FPpart* d_pxx_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_pxy(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_u, FPpart* d_v, 
    FPpart* d_pxy_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_pxz(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_u, FPpart* d_w, 
    FPpart* d_pxz_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_pyy(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_v, 
    FPpart* d_pyy_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_pyz(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_v, FPpart* d_w, 
    FPpart* d_pyz_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
__global__ void interpP2G_kernel_pzz(
    FPpart* d_x, FPpart* d_y, FPpart* d_z, 
    FPpart* d_w, 
    FPpart* d_pzz_flat, FPpart* d_weight,
    FPpart d_invVOL, FPpart d_xStart, FPpart d_yStart, FPpart d_zStart, 
    FPpart d_invdx, FPpart d_invdy, FPpart d_invdz, 
    int d_nxn, int d_nyn, int d_nzn, int d_nop) 
{
...
//     FPpart* s_Jx = &s_rhon[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_Jy = &s_Jx[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_Jz = &s_Jy[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_pxx = &s_Jz[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_pxy = &s_pxx[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_pxz = &s_pxy[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_pyy = &s_pxz[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_pyz = &s_pyy[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
//     FPpart* s_pzz = &s_pyz[(blockDim.x + 2) * (blockDim.y + 2) * (blockDim.z + 2)];
...
//     int shared_idx = local_x * shared_ny * shared_nz + local_y * shared_nz + local_z;

//     // add zero and n+1 
//     if (global_x < d_nxn && global_y < d_nyn && global_z < d_nzn) {
//         int global_idx = global_x * d_nyn * d_nzn + global_y * d_nzn + global_z;
...
//     for (int i = shared_idx; i < d_nop; i += blockDim.x * blockDim.y * blockDim.z) {
//         // arrays needed for interpolation
//         FPpart weight[2][2][2];
//         // FPpart temp[2][2][2];
//         FPpart xi[2], eta[2], zeta[2];
...
//     if (global_x < d_nxn && global_y < d_nyn && global_z < d_nzn) {
//             int global_idx = global_x * d_nyn * d_nzn + global_y * d_nzn + global_z;            
//             atomicAdd(&d_rhon_flat[global_idx], s_rhon[shared_idx]);
//             atomicAdd(&d_Jx_flat[global_idx], s_Jx[shared_idx]);
//             atomicAdd(&d_Jy_flat[global_idx], s_Jy[shared_idx]);
//             atomicAdd(&d_Jz_flat[global_idx], s_Jz[shared_idx]);
//             atomicAdd(&d_pxx_flat[global_idx], s_pxx[shared_idx]);
//             atomicAdd(&d_pxy_flat[global_idx], s_pxy[shared_idx]);
//             atomicAdd(&d_pxz_flat[global_idx], s_pxz[shared_idx]);
//             atomicAdd(&d_pyy_flat[global_idx], s_pyy[shared_idx]);
//             atomicAdd(&d_pyz_flat[global_idx], s_pyz[shared_idx]);
//             atomicAdd(&d_pzz_flat[global_idx], s_pzz[shared_idx]);
//         }
// }

// This is Async V2  
void interpP2G_GPU_V2(struct particles* part, struct interpDensSpecies* ids, struct grid* grd)
{
    // corresponds to XN, YN etc. also Ex, Bxn COPY for slimpicity
    int nxn = grd->nxn; 
    int nyn = grd->nyn;
    ...
    FPpart *d_pxx_flat, *d_pxy_flat, *d_pxz_flat, *d_pyy_flat, *d_pyz_flat, *d_pzz_flat;
    ...
    cudaMemcpyAsync(d_x, part->x, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[0]);
    cudaMemcpyAsync(d_y, part->y, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[1]);
    cudaMemcpyAsync(d_z, part->z, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[2]);
    
    cudaMemcpyAsync(d_q, part->q, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[6]);
    cudaMemcpyAsync(d_XN_flat, XN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice, stream[7]);
    cudaMemcpyAsync(d_YN_flat, YN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice, stream[8]);
    cudaMemcpyAsync(d_ZN_flat, ZN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice, stream[9]);
    ...
    cudaMemcpyAsync(d_u, part->u, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[3]);
    cudaMemcpyAsync(d_v, part->v, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[4]);
    cudaMemcpyAsync(d_w, part->w, nop * sizeof(FPpart), cudaMemcpyHostToDevice, stream[5]);

    cudaMemcpyAsync(d_rhon_flat, rhon_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[0]);
    cudaMemcpyAsync(d_Jx_flat, Jx_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[1]);
    cudaMemcpyAsync(d_Jy_flat, Jy_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[2]);
    cudaMemcpyAsync(d_Jz_flat, Jz_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[3]);
    cudaMemcpyAsync(d_pxx_flat, pxx_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[4]);
    cudaMemcpyAsync(d_pxy_flat, pxy_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[5]);
    cudaMemcpyAsync(d_pxz_flat, pxz_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[6]);
    cudaMemcpyAsync(d_pyy_flat, pyy_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[7]);
    cudaMemcpyAsync(d_pyz_flat, pyz_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[8]);
    cudaMemcpyAsync(d_pzz_flat, pzz_flat, currentStreamSize, cudaMemcpyHostToDevice, stream[9]);

    interpP2G_kernel_rhon<<<blocksPerGrid, threadsPerBlock, 0, stream[0]>>>(
        d_x, d_y, d_z, 
        d_rhon_flat, d_weight,
        grd->invVOL, grd->xStart, grd->yStart, grd->zStart,
        grd->invdx, grd->invdy, grd->invdz, 
        nxn, nyn, nzn, nop
    ); 
    ...
    cudaMemcpyAsync(rhon_flat, d_rhon_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[0]);
    cudaMemcpyAsync(Jx_flat, d_Jx_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[1]);
    cudaMemcpyAsync(Jy_flat, d_Jy_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[2]);
    cudaMemcpyAsync(Jz_flat, d_Jz_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[3]);
    cudaMemcpyAsync(pxx_flat, d_pxx_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[4]);
    cudaMemcpyAsync(pxy_flat, d_pxy_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[5]);
    cudaMemcpyAsync(pxz_flat, d_pxz_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[6]);
    cudaMemcpyAsync(pyy_flat, d_pyy_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[7]);
    cudaMemcpyAsync(pyz_flat, d_pyz_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[8]);
    cudaMemcpyAsync(pzz_flat, d_pzz_flat, currentStreamSize, cudaMemcpyDeviceToHost, stream[9]);
...
//     cudaMemcpy(d_YN_flat, YN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_ZN_flat, ZN_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_rhon_flat, rhon_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_Jx_flat, Jx_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_Jy_flat, Jy_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_Jz_flat, Jz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_pxx_flat, pxx_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_pxy_flat, pxy_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_pxz_flat, pxz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_pyy_flat, pyy_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_pyz_flat, pyz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice);
//     cudaMemcpy(d_pzz_flat, pzz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyHostToDevice); 
...
//     // int shared_mem_size = (8 + 2) * (8 + 2) * (8 + 2) * sizeof(FPpart) * 10;
...
//     // Copy back to host
//     cudaMemcpy(rhon_flat, d_rhon_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(Jx_flat, d_Jx_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(Jy_flat, d_Jy_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(Jz_flat, d_Jz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(pxx_flat, d_pxx_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(pxy_flat, d_pxy_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(pxz_flat, d_pxz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(pyy_flat, d_pyy_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(pyz_flat, d_pyz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
    // cudaMemcpy(pzz_flat, d_pzz_flat, nxn * nyn * nzn * sizeof(FPpart), cudaMemcpyDeviceToHost);
...
// // V1 - not working cba
// void interpP2G_GPU_v1(struct particles* part, struct interpDensSpecies* ids, struct grid* grd) {
//     // Extract dimensions
//     int nxn = grd->nxn; 
//     int nyn = grd->nyn;
//     int nzn = grd->nzn;
...
//         cudaMemcpyAsync(&d_x[offset], &part->x[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);
//         cudaMemcpyAsync(&d_y[offset], &part->y[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);
//         cudaMemcpyAsync(&d_z[offset], &part->z[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);
//         cudaMemcpyAsync(&d_u[offset], &part->u[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);
//         cudaMemcpyAsync(&d_v[offset], &part->v[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);
//         cudaMemcpyAsync(&d_w[offset], &part->w[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);
//         cudaMemcpyAsync(&d_q[offset], &part->q[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyHostToDevice, streams[i]);

//         // Launch kernel
//         int blocksPerGrid = (currentChunkSize + threadsPerBlock - 1) / threadsPerBlock;
//         interpP2G_kernel<<<blocksPerGrid, threadsPerBlock, 0, streams[i]>>>( 
...
//         cudaMemcpyAsync(&ids->rhon_flat[offset], &d_rhon_flat[offset], currentChunkSize * sizeof(FPpart), cudaMemcpyDeviceToHost, streams[i]); }

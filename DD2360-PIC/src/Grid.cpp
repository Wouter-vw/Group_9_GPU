...
void setGrid(struct parameters *param, struct grid *grd) {
  ...
  grd->nodes =
      newArr3<Vec3<FPfield>>(&grd->nodes_flat, grd->nxn, grd->nyn, grd->nzn);

  // calculate the coordinates - Nodes
  for (int i = 0; i < grd->nxn; i++) {
    for (int j = 0; j < grd->nyn; j++) {
      for (int k = 0; k < grd->nzn; k++) {
        grd->nodes[i][j][k].x = (FPfield)(grd->xStart + (i - 1) * grd->dx);
        grd->nodes[i][j][k].y = (FPfield)(grd->yStart + (j - 1) * grd->dy);
        grd->nodes[i][j][k].z = (FPfield)(grd->zStart + (k - 1) * grd->dz);
      }
    }
  }
}
...
void grid_deallocate(struct grid *grd) {
  delArr3(grd->nodes, grd->nxn, grd->nyn);
}
